...
> Un client peut passer plusieurs commandes, ou n'en avoir encore passé aucune ; une commande est passée par un seul client.

Rien n'empêche qu'un client existe dans le système sans avoir encore passé de commande (ex: un client vient d'être enregistré mais n'a pas encore validé d'achat). Le minimum côté Client est donc 0.

**Cardinalité :** `Client (0,n) —— (1,1) Commande`

- Un client : 0 à n commandes (min=0, un client peut exister sans avoir commandé ; max=n, ex. Acme Corp a `CMD-901` et `CMD-903`).
